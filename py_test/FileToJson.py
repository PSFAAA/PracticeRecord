...
class FileToDict:
    ...
    def get_json(self):
        ...
        if file_suffix == 'xlsx':
            ...
            result = json.dumps(self.xlsx_file(), ensure_ascii=False)
        elif file_suffix == 'csv':
            # print(json.dumps(self.csv_file(), ensure_ascii=False))
            result = json.dumps(self.csv_file(), ensure_ascii=False)
        elif file_suffix == 'xls':
            # print(json.dumps(self.xls_file(), ensure_ascii=False))
            result = json.dumps(self.xls_file(), ensure_ascii=False)
        return result
...
def toFile(result):
    ...
    path = r'C:\Users\JX\Desktop'
    if not os.path.exists(path + '/FileToJson'):
        os.makedirs(path + '/FileToJson')
    for name in ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't',
                 'u', 'v', 'w', 'x', 'y', 'z']:
        if not os.path.exists(path + '/FileToJson/' + name + '.txt'):
            with open(path + '/FileToJson/' + name + '.txt', 'a', encoding='utf-8') as f:
                f.write(result)
            print("转换完毕！！！")
            break
